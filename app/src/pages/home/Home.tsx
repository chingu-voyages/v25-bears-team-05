...
import { IThread, IThreadDataProcessed, IThreadReferral } from "../../services/thread/thread.type";
import { addThread } from "../../services/thread";
import Spinner from "../../components/spinner";
import Post from "../../components/post";
import { IConnectionCardInfo, IThreadCardInfo } from "../../components/profileCard/profileCard.type";
import ProfileCard from "../../components/profileCard";
import { getFeed } from "../../services/feed/feed";
import { IFeedItemsProps, IFeedProcessedResponse } from "../../services/feed/feed.type";
...
function Home() {
  ...
  useEffect(() => {
    ...
    getFeed({query: "", onSuccess, onError: (msg) => { setErrorMessage(msg); }});
  }, []);
      ...
      <div className="Home-page__feed">
        <ul className="Home-page__feed__list">
          {feed.map(({suggestion, thread}: IFeedItemsProps, index) => (
            <FeedItem {...{suggestion, thread}} key={"feedItem" + (thread?.threadData?.id || suggestion?.id || index)} />
          ))}
        </ul>
