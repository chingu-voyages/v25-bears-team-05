...
function Search({ searchResults, classNameInfo = ""}: { searchResults: ISearchResults, classNameInfo?: string}) {
  ...
  useEffect(()=> {
    ...
    setPrivateThreads([]);
    (async ()=> {
      if (searchResults.public_threads && searchResults.public_threads.length > 0) {
        const processedThreadData = await Promise
        .all(searchResults.public_threads
          .map((publicThread)=> processThread(publicThread)));
        setPublicThreads([...publicThreads, ...processedThreadData])
      }
      if(searchResults.private_threads && searchResults.private_threads?.length > 0) {
        const processedPrivateThreadData = await Promise
        .all(searchResults.private_threads
          .map((privateThread) => processThread(privateThread)))
  ...
  return (
    <div className={`${classNameInfo} Search-page-visible`}>
     ...
     {publicThreads.length > 0 && publicThreads.map((publicThread)=> (
       <Post {...{threadData: publicThread.threadData!}} key={publicThread.threadData?.id}/>
     ))}
     {privateThreads.length > 0 && privateThreads.map((privateThread)=> (
         <Post {...{threadData: privateThread.threadData!}} key={privateThread.threadData?.id} />
       ))
     }
    </div>
  )
